...
import Dropdown from './new_component/dropdown';
import {ApiClient, BasePropertyProps} from "adminjs";

const api = new ApiClient();

const Multiselect = (props: BasePropertyProps) => {
    ...
    useEffect(() => {
        async function get_Names() {
            ...
            setSelected(response.data.listMark);
    ...
    const sendRequest = async () => {
        const data = {
            ...
            id: record.id,
            list: selectedItems.map((value) => value.id),
        };
        const response = await api.resourceAction({
            resourceId: property.resourceId,
            actionName: "SaveData",
